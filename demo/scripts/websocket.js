...
$(function() {
    ...
    var wwsConsoleLog = document.getElementById("wwsConsoleLog");

    function wwsLogToConsole(message, ignoreSecureTag) {

        var pre = document.createElement("p");
        pre.style.wordWrap = "break-word";
        pre.innerHTML = "";
        pre.innerHTML = pre.innerHTML + message;
        wwsConsoleLog.appendChild(pre);

        while (wwsConsoleLog.childNodes.length > 50) {
            wwsConsoleLog.removeChild(wwsConsoleLog.firstChild);
        }

        wwsConsoleLog.scrollTop = wwsConsoleLog.scrollHeight;
    }

    $('#wwsClearLogBut').click(function() {
        while (wwsConsoleLog.childNodes.length) {
            wwsConsoleLog.removeChild(wwsConsoleLog.lastChild);
        }
    });

    var echoUri = 'http://httpbin.org/post';
    $('#wwsUri').val(echoUri);

    $('#wwsSendBut').click(function() {
        var message = $('#wwsMessage').val();
        wwsLogToConsole("SENT: " + message);
        $.ajax({
            url: echoUri,
            type: 'POST',
            dataType: 'json',
            ...
            success: function(data, textStatus, jqXHR) {
                console.log(data);
                wwsLogToConsole('<span style="color: blue;">RESPONSE: ' + data.form.requestParam + '</span>');
            },
